...
def excel_to_list(data_file, sheet):
    ...
    ex = xlrd.open_workbook(data_file)  # 打开excel
    sh = ex.sheet_by_name(sheet)  # 获取工作簿
...
def get_test_data(data_list, case_name):
    for case_data in data_list:    #data_list把值赋予case_data，
        if case_name == case_data['case_name']:  # 如果字典数据中case_name与参数一致
            return case_data
            #如果查询不到会返回None

if __name__ == '__main__':   # 测试一下自己的代码
    data_list = excel_to_list("../data/test_data(mac).xls", "TestUserLogin")  # 读取excel，TestUserLogin工作簿的所有数据
    case_data = get_test_data(data_list, 'test_user_login_normal')  # 查找用例'test_user_login_normal'的数据
    # print(data_list)
    print(case_data)
    data = json.dumps(json.loads(case_data.get('data')))
    print(data)