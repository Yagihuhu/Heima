...
class TestUserLogin(unittest.TestCase):  # 类必须Test开头，继承TestCase才能识别为用例类
    ...
    def setUpClass(cls):  # 整个测试类只执行一次
        cls.data_list = excel_to_list(os.path.join(data_path, "test_data(agent).xls"), "TestLogin")  # 增加data路径

    def test_user_login_normal(self):
        case_data = get_test_data(self.data_list, 'test_login32')
        ...
        res = requests.post(url=url, data=json.dumps(data), headers=json.loads(headers),verify=False)   #data字典转json
        res_code = res.json()['code']  # 获取状态码
        # print("res_code:",res_code)
        res_com1 = (res.json()['data']['company_id'])
        res_com =  res_com1.encode()
        with open(com_id_path,'wb') as f:
            f.write(res_com)
