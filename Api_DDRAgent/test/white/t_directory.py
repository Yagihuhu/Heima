...
class TestConditionAdd(unittest.TestCase):  # 类必须Test开头，继承TestCase才能识别为用例类
    ...
    def setUpClass(cls):  # 整个测试类只执行一次
        cls.data_list = excel_to_list(os.path.join(data_path, "test_data.xls"), "TestWhite")  # 增加data路径

    def test_add_condition(self):
        case_data = get_test_data(self.data_list, 'test_directory_add')  # 从数据列表中查找到该用例数据
